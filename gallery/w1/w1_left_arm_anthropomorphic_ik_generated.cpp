...
struct w1_left_arm_anthropomorphic_ik {
...
void test_ik_solve_w1_left_arm_anthropomorphic()
{
    std::array<double, w1_left_arm_anthropomorphic_ik::robot_nq> theta;
    std::random_device rd;
    std::uniform_real_distribution<double> distribution;
    for(auto i = 0; i < theta.size(); i++)
        theta[i] = distribution(rd);
    const Eigen::Matrix4d ee_pose = w1_left_arm_anthropomorphic_ik::computeFK(theta);
    auto ik_output = w1_left_arm_anthropomorphic_ik::computeIK(ee_pose, theta[4]);
    for(int i = 0; i < ik_output.size(); i++)
    {
        Eigen::Matrix4d ee_pose_i = w1_left_arm_anthropomorphic_ik::computeFK(ik_output[i]);
