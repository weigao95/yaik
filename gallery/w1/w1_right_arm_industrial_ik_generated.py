...
def w1_right_arm_industrial_ik_target_original_to_raw(T_ee: np.ndarray):
    r_11: float = T_ee[0, 0]
    r_12: float = T_ee[0, 1]
    r_13: float = T_ee[0, 2]
    Px: float = T_ee[0, 3]
...
def w1_right_arm_industrial_ik_target_raw_to_original(T_ee: np.ndarray):
    r_11: float = T_ee[0, 0]
    r_12: float = T_ee[0, 1]
    r_13: float = T_ee[0, 2]
    Px: float = T_ee[0, 3]
...
def w1_right_arm_industrial_fk(theta_input: np.ndarray):
    ...
    ee_pose[0, 2] = x26
    ee_pose[0, 3] = -d_2*x13 + d_4*x22 + post_transform_s4*x26
    ee_pose[1, 0] = -x0*x31 + x20*x34
    ee_pose[1, 1] = -x0*x34 - x20*x31
    ee_pose[1, 2] = x35
    ee_pose[1, 3] = -d_2*x28 + d_4*x32 + post_transform_s4*x35
    ee_pose[2, 0] = -x0*x39 + x20*x42
    ee_pose[2, 1] = -x0*x42 - x20*x39
    ee_pose[2, 2] = x43
    ee_pose[2, 3] = -d_2*x5 + d_4*x40 + post_transform_s4*x43 + pre_transform_s0
    return ee_pose


def w1_right_arm_industrial_twist_jacobian(theta_input: np.ndarray):
    th_0 = theta_input[0] + th_0_offset_original2raw
    th_1 = theta_input[1] + th_1_offset_original2raw
    th_2 = theta_input[2] + th_2_offset_original2raw
    th_3 = theta_input[3] + th_3_offset_original2raw
...
def w1_right_arm_industrial_angular_velocity_jacobian(theta_input: np.ndarray):
    th_0 = theta_input[0] + th_0_offset_original2raw
    th_1 = theta_input[1] + th_1_offset_original2raw
    th_2 = theta_input[2] + th_2_offset_original2raw
    th_3 = theta_input[3] + th_3_offset_original2raw
...
def w1_right_arm_industrial_transform_point_jacobian(theta_input: np.ndarray, point_on_ee: np.ndarray):
    th_0 = theta_input[0] + th_0_offset_original2raw
    th_1 = theta_input[1] + th_1_offset_original2raw
    th_2 = theta_input[2] + th_2_offset_original2raw
    th_3 = theta_input[3] + th_3_offset_original2raw
...
def w1_right_arm_industrial_ik_solve_raw(T_ee: np.ndarray, th_4):
...
def w1_right_arm_industrial_ik_solve(T_ee: np.ndarray, th_4):
    T_ee_raw_in = w1_right_arm_industrial_ik_target_original_to_raw(T_ee)
    ik_output_raw = w1_right_arm_industrial_ik_solve_raw(T_ee_raw_in, th_4 + th_4_offset_original2raw)
    ik_output = list()
    for i in range(len(ik_output_raw)):
        ...
        ee_pose_i = w1_right_arm_industrial_fk(ik_out_i)
...
def test_ik_solve_w1_right_arm_industrial():
    theta_in = np.random.random(size=(7, ))
    ee_pose = w1_right_arm_industrial_fk(theta_in)
    ik_output = w1_right_arm_industrial_ik_solve(ee_pose, th_4=theta_in[4])
    for i in range(len(ik_output)):
        ee_pose_i = w1_right_arm_industrial_fk(ik_output[i])
