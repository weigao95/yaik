...
struct w1_right_arm_anthropomorphic_ik {
...
// Robot parameters
static constexpr double d_2 = 0.26;
static constexpr double d_4 = 0.264;
static constexpr double post_transform_s4 = -0.066;
static constexpr double pre_transform_s0 = 0.1025;
...
static void computeTwistJacobian(const std::array<double, robot_nq>& theta_input_original, Eigen::Matrix<double, 6, robot_nq>& jacobian)
{
    ...
    const double x37 = pre_transform_s0 - x36;
    const double x38 = d_2*std::pow(x1, 2);
    const double x39 = d_4*x32 + x37;
    const double x40 = d_2*x23;
    const double x41 = d_4*x26 - x40;
    const double x42 = d_2*x3;
    const double x43 = d_4*x15 - x42;
    // End of temp variables
    ...
    jacobian(2, 5) = x34;
    jacobian(2, 6) = x35;
    jacobian(3, 1) = -pre_transform_s0*x2;
    jacobian(3, 2) = x23*x36 + x23*x37;
    jacobian(3, 3) = -x12*x38 - x24*x37;
    jacobian(3, 4) = -x26*x39 + x32*x41;
    jacobian(3, 5) = -x28*x39 + x34*x41;
    jacobian(3, 6) = -x29*x39 + x35*x41;
    jacobian(4, 1) = -pre_transform_s0*x0;
    jacobian(4, 2) = -x3*x36 - x3*x37;
    jacobian(4, 3) = x37*x9 + x38*x8;
    jacobian(4, 4) = x15*x39 - x32*x43;
    jacobian(4, 5) = x19*x39 - x34*x43;
    jacobian(4, 6) = x22*x39 - x35*x43;
    jacobian(5, 3) = -x24*x42 + x40*x9;
    jacobian(5, 4) = -x15*x41 + x26*x43;
    jacobian(5, 5) = -x19*x41 + x28*x43;
    jacobian(5, 6) = -x22*x41 + x29*x43;
    return;
}
...
static void computeTransformPointJacobian(const std::array<double, robot_nq>& theta_input_original, Eigen::Vector3d& point_on_ee, Eigen::Matrix<double, 3, robot_nq>& jacobian)
{
    ...
    const double x24 = d_4*x19 + x8;
    const double x25 = d_2*x7;
    const double x26 = d_4*x23 - x25;
    ...
    const double x37 = x0*x3;
    const double x38 = -x2*x20 + x21;
    const double x39 = x11 + x14*x2;
    const double x40 = x16*x37 - x17*x39;
    const double x41 = d_2*x37;
    const double x42 = d_4*x40 - x41;
    const double x43 = x16*x39 + x17*x37;
    const double x44 = -x27*x38 - x28*x43;
    const double x45 = -x33*x40 - x34*(x27*x43 - x28*x38);
    // End of temp variables
    ...
    jacobian(0, 6) = -p_on_ee_y*x35 + p_on_ee_z*x36 - x24*x36 + x26*x35;
    jacobian(1, 0) = p_on_ee_x;
    jacobian(1, 1) = -pre_transform_s0*x4 + x5;
    jacobian(1, 2) = -p_on_ee_x*x2 + x1*x3 - x37*x6 - x37*x8;
    jacobian(1, 3) = p_on_ee_x*x10 - p_on_ee_z*x38 + x12*x20 + x38*x8;
    jacobian(1, 4) = p_on_ee_x*x19 - p_on_ee_z*x40 - x19*x42 + x24*x40;
    jacobian(1, 5) = p_on_ee_x*x30 - p_on_ee_z*x44 + x24*x44 - x30*x42;
    jacobian(1, 6) = p_on_ee_x*x35 - p_on_ee_z*x45 + x24*x45 - x35*x42;
    jacobian(2, 1) = -p_on_ee_x*x0 - p_on_ee_y*x4;
    jacobian(2, 2) = p_on_ee_x*x7 - p_on_ee_y*x37;
    jacobian(2, 3) = -p_on_ee_x*x15 + p_on_ee_y*x38 - x15*x41 + x25*x38;
    jacobian(2, 4) = -p_on_ee_x*x23 + p_on_ee_y*x40 + x23*x42 - x26*x40;
    jacobian(2, 5) = -p_on_ee_x*x32 + p_on_ee_y*x44 - x26*x44 + x32*x42;
    jacobian(2, 6) = -p_on_ee_x*x36 + p_on_ee_y*x45 - x26*x45 + x36*x42;
    return;
}
...
static void computeRawIK(const Eigen::Matrix4d& T_ee, double th_4, SolutionQueue<intermediate_solution_size, max_n_solutions>& solution_queue, NodeIndexWorkspace<max_n_solutions>& node_index_workspace, std::vector<std::array<double, robot_nq>>& ik_output)
{
    ...
    auto ExplicitSolutionNode_node_1_solve_th_3_processor = [&]() -> void
    {
        ...
        const bool condition_0 = (1.0/2.0)*std::fabs((-std::pow(d_2, 2) - std::pow(d_4, 2) + std::pow(inv_Px, 2) + std::pow(inv_Py, 2) + std::pow(inv_Pz, 2))/(d_2*d_4)) <= 1;
        if (condition_0)
        {
            // Temp variable for efficiency
            const double x0 = safe_acos((1.0/2.0)*(std::pow(d_2, 2) + std::pow(d_4, 2) - std::pow(inv_Px, 2) - std::pow(inv_Py, 2) - std::pow(inv_Pz, 2))/(d_2*d_4));
            // End of temp variables
        ...
        const bool condition_1 = (1.0/2.0)*std::fabs((-std::pow(d_2, 2) - std::pow(d_4, 2) + std::pow(inv_Px, 2) + std::pow(inv_Py, 2) + std::pow(inv_Pz, 2))/(d_2*d_4)) <= 1;
        if (condition_1)
        {
            // Temp variable for efficiency
            const double x0 = safe_acos((1.0/2.0)*(std::pow(d_2, 2) + std::pow(d_4, 2) - std::pow(inv_Px, 2) - std::pow(inv_Py, 2) - std::pow(inv_Pz, 2))/(d_2*d_4));
    ...
    auto EquationAllZeroDispatcherNode_node_2_processor = [&]() -> void
    {
        ...
        for(int i = 0; i < this_node_input_index.size(); i++)
        {
            ...
            const bool checked_result = std::fabs(inv_Pz) <= 9.9999999999999995e-7 && std::fabs(d_2*std::sin(th_3)*std::cos(th_4)) <= 9.9999999999999995e-7 && std::fabs(d_2*std::cos(th_3) - d_4) <= 9.9999999999999995e-7;
    ...
    auto ExplicitSolutionNode_node_3_solve_th_5_processor = [&]() -> void
    {
        ...
        for(int i = 0; i < this_node_input_index.size(); i++)
        {
            ...
            const bool condition_0 = std::fabs(inv_Pz) >= zero_tolerance || std::fabs(d_2*std::sin(th_3)*std::cos(th_4)) >= zero_tolerance || std::fabs(d_2*std::cos(th_3) - d_4) >= zero_tolerance;
            if (condition_0)
            {
                // Temp variable for efficiency
                const double x0 = std::sin(th_3);
                const double x1 = std::cos(th_4);
                const double x2 = -d_2*std::cos(th_3) + d_4;
                const double x3 = std::atan2(-d_2*x0*x1, x2);
                const double x4 = safe_sqrt(std::pow(d_2, 2)*std::pow(x0, 2)*std::pow(x1, 2) - std::pow(inv_Pz, 2) + std::pow(x2, 2));
                // End of temp variables
                RawSolution solution_0(this_solution);
                solution_0[6] = x3 + std::atan2(x4, inv_Pz);
                int appended_idx = append_solution_to_queue(solution_0);
                add_input_index_to(4, appended_idx);
            }
            
            const bool condition_1 = std::fabs(inv_Pz) >= zero_tolerance || std::fabs(d_2*std::sin(th_3)*std::cos(th_4)) >= zero_tolerance || std::fabs(d_2*std::cos(th_3) - d_4) >= zero_tolerance;
            if (condition_1)
            {
                // Temp variable for efficiency
                const double x0 = std::sin(th_3);
                ...
                const double x4 = safe_sqrt(std::pow(d_2, 2)*std::pow(x0, 2)*std::pow(x1, 2) - std::pow(inv_Pz, 2) + std::pow(x2, 2));
                // End of temp variables
                const double tmp_sol_value = x3 + std::atan2(-x4, inv_Pz);
    ...
    auto EquationAllZeroDispatcherNode_node_4_processor = [&]() -> void
    {
        ...
        for(int i = 0; i < this_node_input_index.size(); i++)
        {
            ...
            const bool checked_result = std::fabs(d_2*std::sin(th_3)*std::sin(th_4)) <= 9.9999999999999995e-7 && std::fabs(d_2*std::sin(th_3)*std::cos(th_4)*std::cos(th_5) - d_2*std::sin(th_5)*std::cos(th_3) + d_4*std::sin(th_5)) <= 9.9999999999999995e-7;
            if (!checked_result)  // To non-degenerate node
    ...
    auto ExplicitSolutionNode_node_5_solve_th_6_processor = [&]() -> void
    {
        ...
        for(int i = 0; i < this_node_input_index.size(); i++)
        {
            ...
            const bool condition_0 = std::fabs(d_2*std::sin(th_3)*std::sin(th_4)) >= 9.9999999999999995e-7 || std::fabs(d_2*std::sin(th_3)*std::cos(th_4)*std::cos(th_5) - d_2*std::sin(th_5)*std::cos(th_3) + d_4*std::sin(th_5)) >= 9.9999999999999995e-7;
            if (condition_0)
            {
                // Temp variable for efficiency
                const double x0 = d_2*std::sin(th_3);
                const double x1 = x0*std::sin(th_4);
                const double x2 = std::sin(th_5);
                const double x3 = -d_2*x2*std::cos(th_3) + d_4*x2 + x0*std::cos(th_4)*std::cos(th_5);
                // End of temp variables
                const double tmp_sol_value = std::atan2(inv_Px*x1 - inv_Py*x3, inv_Px*x3 + inv_Py*x1);
    ...
    auto ExplicitSolutionNode_node_7_solve_th_1_processor = [&]() -> void
    {
        ...
        for(int i = 0; i < this_node_input_index.size(); i++)
        {
            ...
            const auto& this_solution = solution_queue.get_solution(node_input_i_idx_in_queue);
            const double th_5 = this_solution[6];
            const double th_6 = this_solution[7];
            
            const bool condition_0 = std::fabs((Pz + d_4*(r_33*std::cos(th_5) + (r_31*std::cos(th_6) - r_32*std::sin(th_6))*std::sin(th_5)))/d_2) <= 1;
            if (condition_0)
            {
                // Temp variable for efficiency
                const double x0 = safe_acos((-Pz - d_4*(r_33*std::cos(th_5) + (r_31*std::cos(th_6) - r_32*std::sin(th_6))*std::sin(th_5)))/d_2);
                // End of temp variables
                RawSolution solution_0(this_solution);
                solution_0[3] = x0;
                int appended_idx = append_solution_to_queue(solution_0);
            ...
            const bool condition_1 = std::fabs((Pz + d_4*(r_33*std::cos(th_5) + (r_31*std::cos(th_6) - r_32*std::sin(th_6))*std::sin(th_5)))/d_2) <= 1;
            if (condition_1)
            {
                // Temp variable for efficiency
                const double x0 = safe_acos((-Pz - d_4*(r_33*std::cos(th_5) + (r_31*std::cos(th_6) - r_32*std::sin(th_6))*std::sin(th_5)))/d_2);
    ...
    auto SolvedVariableDispatcherNode_node_8_processor = [&]() -> void
    {
        ...
        for(int i = 0; i < this_node_input_index.size(); i++)
        {
            ...
            const bool degenerate_valid_0 = std::fabs(th_1) <= 9.9999999999999995e-7;
            ...
            const bool degenerate_valid_1 = std::fabs(th_1 - M_PI) <= 9.9999999999999995e-7;
    ...
    auto ExplicitSolutionNode_node_17_solve_th_0th_2_soa_processor = [&]() -> void
    {
        ...
        for(int i = 0; i < this_node_input_index.size(); i++)
        {
            ...
            const bool condition_0 = std::fabs(Px) >= zero_tolerance || std::fabs(Py) >= zero_tolerance || std::fabs(d_4*std::sin(th_3)) >= zero_tolerance;
            if (condition_0)
            {
                // Temp variable for efficiency
                const double x0 = 1/(d_4*std::sin(th_3));
                // End of temp variables
                const double tmp_sol_value = std::atan2(Py*x0, Px*x0);
    ...
    auto ExplicitSolutionNode_node_19_solve_th_0_processor = [&]() -> void
    {
    ...
    auto ExplicitSolutionNode_node_21_solve_th_2_processor = [&]() -> void
    {
        ...
        for(int i = 0; i < this_node_input_index.size(); i++)
        {
            ...
            const double th_0th_2_soa = this_solution[2];
            ...
            if (condition_0)
            {
                // Temp variable for efficiency
                // End of temp variables
                const double tmp_sol_value = -th_0 + th_0th_2_soa;
    ...
    auto ExplicitSolutionNode_node_12_solve_negative_th_2_positive_th_0__soa_processor = [&]() -> void
    {
    ...
    auto ExplicitSolutionNode_node_14_solve_th_0_processor = [&]() -> void
    {
    ...
    auto ExplicitSolutionNode_node_16_solve_th_2_processor = [&]() -> void
    {
        ...
        for(int i = 0; i < this_node_input_index.size(); i++)
        {
            ...
            if (!solution_queue.is_solution_valid(node_input_i_idx_in_queue))
                continue;
            const auto& this_solution = solution_queue.get_solution(node_input_i_idx_in_queue);
            const double negative_th_2_positive_th_0__soa = this_solution[0];
            const double th_0 = this_solution[1];
            ...
            if (condition_0)
            {
                // Temp variable for efficiency
                // End of temp variables
                const double tmp_sol_value = -negative_th_2_positive_th_0__soa + th_0;
                solution_queue.get_solution(node_input_i_idx_in_queue)[4] = tmp_sol_value;
            }
            else
            {
    ...
    auto ExplicitSolutionNode_node_9_solve_th_0_processor = [&]() -> void
    {
        ...
        for(int i = 0; i < this_node_input_index.size(); i++)
        {
            ...
            const bool condition_0 = std::fabs(d_2*std::sin(th_1)) >= zero_tolerance || std::fabs(-Px + d_4*(-r_13*std::cos(th_5) - (r_11*std::cos(th_6) - r_12*std::sin(th_6))*std::sin(th_5))) >= zero_tolerance || std::fabs(-Py + d_4*(-r_23*std::cos(th_5) - (r_21*std::cos(th_6) - r_22*std::sin(th_6))*std::sin(th_5))) >= zero_tolerance;
            if (condition_0)
            {
                // Temp variable for efficiency
                const double x0 = std::cos(th_5);
                const double x1 = std::sin(th_5);
                const double x2 = std::cos(th_6);
                const double x3 = std::sin(th_6);
                const double x4 = 1/(d_2*std::sin(th_1));
                // End of temp variables
                const double tmp_sol_value = std::atan2(x4*(-Py + d_4*(-r_23*x0 - x1*(r_21*x2 - r_22*x3))), x4*(-Px + d_4*(-r_13*x0 - x1*(r_11*x2 - r_12*x3))));
                solution_queue.get_solution(node_input_i_idx_in_queue)[1] = tmp_sol_value;
    ...
    auto ExplicitSolutionNode_node_23_solve_th_2_processor = [&]() -> void
    {
        ...
        for(int i = 0; i < this_node_input_index.size(); i++)
        {
            ...
            const bool condition_0 = std::fabs(r_13*std::sin(th_0) - r_23*std::cos(th_0)) >= 9.9999999999999995e-7 || std::fabs(r_13*std::cos(th_0)*std::cos(th_1) + r_23*std::sin(th_0)*std::cos(th_1) - r_33*std::sin(th_1)) >= 9.9999999999999995e-7;
            if (condition_0)
            {
                // Temp variable for efficiency
                ...
                const double x4 = x3*(r_13*x1 - r_23*x2);
                const double x5 = std::sin(th_4);
                const double x6 = std::cos(th_1);
                const double x7 = x3*(r_13*x2*x6 + r_23*x1*x6 - r_33*std::sin(th_1));
                // End of temp variables
                const double tmp_sol_value = std::atan2(x0*x4 - x5*x7, x0*x7 + x4*x5);
                solution_queue.get_solution(node_input_i_idx_in_queue)[4] = tmp_sol_value;
            }
            else
            {
    ...
    auto ExplicitSolutionNode_node_22_solve_th_2_processor = [&]() -> void
    {
        ...
        for(int i = 0; i < this_node_input_index.size(); i++)
        {
            ...
            if (condition_0)
            {
                // Temp variable for efficiency
                const double x0 = std::cos(th_0);
                const double x1 = r_23*x0;
                const double x2 = std::sin(th_0);
                const double x3 = r_13*x2;
                const double x4 = std::sin(th_5);
                const double x5 = x4*std::cos(th_4);
                const double x6 = std::cos(th_1);
                const double x7 = -r_13*x0*x6 - r_23*x2*x6 + r_33*std::sin(th_1);
                const double x8 = x4*std::sin(th_4);
                // End of temp variables
                const double tmp_sol_value = std::atan2(x5*(x1 - x3) + x7*x8, x5*x7 + x8*(-x1 + x3));
                solution_queue.get_solution(node_input_i_idx_in_queue)[4] = tmp_sol_value;
            }
            else
            {
    ...
    auto ExplicitSolutionNode_node_11_solve_th_2_processor = [&]() -> void
    {
        ...
        for(int i = 0; i < this_node_input_index.size(); i++)
        {
            ...
            const bool condition_0 = std::fabs(d_4*std::sin(th_3)) >= zero_tolerance || std::fabs(Px*std::sin(th_0) - Py*std::cos(th_0)) >= zero_tolerance || std::fabs(Px*std::cos(th_0)*std::cos(th_1) + Py*std::sin(th_0)*std::cos(th_1) - Pz*std::sin(th_1)) >= zero_tolerance;
            if (condition_0)
            {
                // Temp variable for efficiency
                const double x0 = std::cos(th_0);
                const double x1 = std::sin(th_0);
                const double x2 = 1/(d_4*std::sin(th_3));
                const double x3 = std::cos(th_1);
                // End of temp variables
                const double tmp_sol_value = std::atan2(x2*(-Px*x1 + Py*x0), x2*(-Px*x0*x3 - Py*x1*x3 + Pz*std::sin(th_1)));
                solution_queue.get_solution(node_input_i_idx_in_queue)[4] = tmp_sol_value;
            }
            else
            {
...
void test_ik_solve_w1_right_arm_anthropomorphic()
{
    std::array<double, w1_right_arm_anthropomorphic_ik::robot_nq> theta;
    std::random_device rd;
    std::uniform_real_distribution<double> distribution;
    for(auto i = 0; i < theta.size(); i++)
        theta[i] = distribution(rd);
    const Eigen::Matrix4d ee_pose = w1_right_arm_anthropomorphic_ik::computeFK(theta);
    auto ik_output = w1_right_arm_anthropomorphic_ik::computeIK(ee_pose, theta[4]);
    for(int i = 0; i < ik_output.size(); i++)
    {
        Eigen::Matrix4d ee_pose_i = w1_right_arm_anthropomorphic_ik::computeFK(ik_output[i]);
...
int main()
{
    test_ik_solve_w1_right_arm_anthropomorphic();
}
