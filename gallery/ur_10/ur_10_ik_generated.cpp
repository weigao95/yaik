...
// Robot parameters
static constexpr double a_1 = 0.612;
static constexpr double a_3 = 0.5723;
static constexpr double d_0 = 0.220941;
static constexpr double d_2 = -0.1719;
static constexpr double d_4 = 0.11485899999999999;
static constexpr double d_5 = 0.1157;
static constexpr double d_6 = 0.0922;
static constexpr double pre_transform_special_symbol_23 = 0.1273;
...
void test_ik_solve_ur_10()
{
    std::array<double, ur_10_ik::robot_nq> theta;
    std::random_device rd;
    std::uniform_real_distribution<double> distribution;
    for(auto i = 0; i < theta.size(); i++)
        theta[i] = distribution(rd);
    const Eigen::Matrix4d ee_pose = ur_10_ik::computeFK(theta);
    auto ik_output = ur_10_ik::computeIK(ee_pose);
    for(int i = 0; i < ik_output.size(); i++)
    {
        Eigen::Matrix4d ee_pose_i = ur_10_ik::computeFK(ik_output[i]);
