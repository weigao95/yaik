...
def rainbow_y1_r_arm_ik_solve_raw(T_ee: np.ndarray, th_0):
    ...
    def ExplicitSolutionNode_node_1_solve_th_3_processor():
        ...
        if condition_0:
            ...
            solution_0[8] = x5 + math.atan2(x7, x6)
        ...
        if condition_1:
            ...
            solution_1[8] = x5 + math.atan2(-x7, x6)
    ...
    def SolvedVariableDispatcherNode_node_2_processor():
        ...
        for i in range(len(this_node_input_index)):
            ...
            th_3 = this_solution[8]
            degenerate_valid_0 = (abs(th_3 - 2.91789252648882 + math.pi) <= 1.0e-6)
    ...
    def ExplicitSolutionNode_node_29_solve_th_1_processor():
        ...
        for i in range(len(this_node_input_index)):
            ...
            if not queue_element_validity[node_input_i_idx_in_queue]:
                continue
            this_solution = solution_queue[node_input_i_idx_in_queue]
            condition_0: bool = (abs(Pz) >= zero_tolerance) or (2*abs(d_2) >= zero_tolerance) or (abs(Px*math.cos(th_0) + Py*math.sin(th_0)) >= zero_tolerance)
            if condition_0:
                # Temp variable for efficiency
                x0 = (1/2)/d_2
                # End of temp variables
                this_solution[2] = math.atan2(x0*(-Px*math.cos(th_0) - Py*math.sin(th_0)), -Pz*x0)
    ...
    def ExplicitSolutionNode_node_31_solve_th_5_processor():
        ...
        for i in range(len(this_node_input_index)):
            ...
            if not queue_element_validity[node_input_i_idx_in_queue]:
                continue
            this_solution = solution_queue[node_input_i_idx_in_queue]
            condition_0: bool = ((1/2)*abs(inv_Pz/d_2) <= 1)
            if condition_0:
                # Temp variable for efficiency
                x0 = safe_acos((1/2)*inv_Pz/d_2)
                # End of temp variables
            ...
            condition_1: bool = ((1/2)*abs(inv_Pz/d_2) <= 1)
            if condition_1:
                # Temp variable for efficiency
                x0 = safe_acos((1/2)*inv_Pz/d_2)
                # End of temp variables
    ...
    def SolvedVariableDispatcherNode_node_32_processor():
    ...
    def ExplicitSolutionNode_node_33_solve_th_6_processor():
        ...
        for i in range(len(this_node_input_index)):
            ...
            this_solution = solution_queue[node_input_i_idx_in_queue]
            th_5 = this_solution[11]
            condition_0: bool = (abs(inv_Px) >= zero_tolerance) or (abs(inv_Py) >= zero_tolerance) or (2*abs(d_2*math.sin(th_5)) >= zero_tolerance)
            if condition_0:
                # Temp variable for efficiency
                x0 = (1/2)/(d_2*math.sin(th_5))
                # End of temp variables
                this_solution[12] = math.atan2(-inv_Py*x0, inv_Px*x0)
                solution_queue[node_input_i_idx_in_queue] = this_solution
    ...
    def SolvedVariableDispatcherNode_node_34_processor():
    ...
    def ExplicitSolutionNode_node_35_solve_th_2th_4_soa_processor():
        ...
        for i in range(len(this_node_input_index)):
            ...
            if condition_0:
                ...
                x3 = math.cos(th_1)
                # End of temp variables
                this_solution[5] = math.atan2(x0*(r_13*x1 - r_23*x2), x0*(r_13*x2*x3 + r_23*x1*x3 - r_33*math.sin(th_1)))
                solution_queue[node_input_i_idx_in_queue] = this_solution
    ...
    def ExplicitSolutionNode_node_37_solve_th_2_processor():
    ...
    def ExplicitSolutionNode_node_39_solve_th_4_processor():
        ...
        for i in range(len(this_node_input_index)):
            ...
            th_2th_4_soa = this_solution[5]
            condition_0: bool = (1 >= zero_tolerance)
            if condition_0:
                # Temp variable for efficiency
                # End of temp variables
                this_solution[9] = -th_2 + th_2th_4_soa
                solution_queue[node_input_i_idx_in_queue] = this_solution
            else:
                queue_element_validity[node_input_i_idx_in_queue] = False
    ...
    def ExplicitSolutionNode_node_12_solve_th_2_processor():
        ...
        for i in range(len(this_node_input_index)):
            ...
            if not queue_element_validity[node_input_i_idx_in_queue]:
                continue
            this_solution = solution_queue[node_input_i_idx_in_queue]
            condition_0: bool = (abs((Px*math.sin(th_0) - Py*math.cos(th_0))/(1.97508329336116*a_3 + 0.221839065558681*d_2)) <= 1)
            if condition_0:
                # Temp variable for efficiency
                x0 = safe_asin((Px*math.sin(th_0) - Py*math.cos(th_0))/(1.97508329336116*a_3 + 0.221839065558681*d_2))
                # End of temp variables
                solution_0: IkSolution = copy.copy(this_solution)
                solution_0[4] = x0
                appended_idx = append_solution_to_queue(solution_0)
            ...
            condition_1: bool = (abs((Px*math.sin(th_0) - Py*math.cos(th_0))/(1.97508329336116*a_3 + 0.221839065558681*d_2)) <= 1)
            if condition_1:
                # Temp variable for efficiency
                x0 = safe_asin((Px*math.sin(th_0) - Py*math.cos(th_0))/(1.97508329336116*a_3 + 0.221839065558681*d_2))
                # End of temp variables
    ...
    def ExplicitSolutionNode_node_14_solve_th_1_processor():
        ...
        for i in range(len(this_node_input_index)):
            ...
            if not queue_element_validity[node_input_i_idx_in_queue]:
                continue
            this_solution = solution_queue[node_input_i_idx_in_queue]
            condition_0: bool = (abs(Pz) >= zero_tolerance) or (abs(0.221839065558681*a_3 + 0.0249167066388374*d_2) >= zero_tolerance) or (abs(Px*math.cos(th_0) + Py*math.sin(th_0)) >= zero_tolerance)
            if condition_0:
                # Temp variable for efficiency
                x0 = Px*math.cos(th_0) + Py*math.sin(th_0)
                x1 = math.atan2(x0, Pz)
                x2 = safe_sqrt(Pz**2 + x0**2 - 0.049212571007949*(-a_3 - 0.11231884057971*d_2)**2)
                x3 = -0.221839065558681*a_3 - 0.0249167066388374*d_2
                # End of temp variables
                solution_0: IkSolution = copy.copy(this_solution)
                solution_0[2] = x1 + math.atan2(x2, x3)
                appended_idx = append_solution_to_queue(solution_0)
                add_input_index_to(15, appended_idx)
                
            condition_1: bool = (abs(Pz) >= zero_tolerance) or (abs(0.221839065558681*a_3 + 0.0249167066388374*d_2) >= zero_tolerance) or (abs(Px*math.cos(th_0) + Py*math.sin(th_0)) >= zero_tolerance)
            if condition_1:
                # Temp variable for efficiency
                x0 = Px*math.cos(th_0) + Py*math.sin(th_0)
                x1 = math.atan2(x0, Pz)
                ...
                x3 = -0.221839065558681*a_3 - 0.0249167066388374*d_2
                # End of temp variables
                this_solution[2] = x1 + math.atan2(-x2, x3)
                solution_queue[node_input_i_idx_in_queue] = this_solution
    ...
    def ExplicitSolutionNode_node_16_solve_th_5_processor():
        ...
        for i in range(len(this_node_input_index)):
            ...
            th_1 = this_solution[2]
            th_2 = this_solution[4]
            condition_0: bool = (abs(-r_13*((0.975083293361163*math.sin(th_1) + 0.221839065558681*math.cos(th_1)*math.cos(th_2))*math.cos(th_0) + 0.221839065558681*math.sin(th_0)*math.sin(th_2)) - r_23*((0.975083293361163*math.sin(th_1) + 0.221839065558681*math.cos(th_1)*math.cos(th_2))*math.sin(th_0) - 0.221839065558681*math.sin(th_2)*math.cos(th_0)) - r_33*(-0.221839065558681*math.sin(th_1)*math.cos(th_2) + 0.975083293361163*math.cos(th_1))) <= 1)
            if condition_0:
                # Temp variable for efficiency
                x0 = math.cos(th_1)
                x1 = math.sin(th_1)
                x2 = 0.221839065558681*math.cos(th_2)
                x3 = math.sin(th_0)
                x4 = 0.221839065558681*math.sin(th_2)
                x5 = math.cos(th_0)
                x6 = x0*x2 + 0.975083293361163*x1
                x7 = safe_acos(-r_13*(x3*x4 + x5*x6) - r_23*(x3*x6 - x4*x5) - r_33*(0.975083293361163*x0 - x1*x2))
                # End of temp variables
                solution_0: IkSolution = copy.copy(this_solution)
                solution_0[11] = x7
                appended_idx = append_solution_to_queue(solution_0)
    ...
    def ExplicitSolutionNode_node_55_solve_th_4th_6_soa_processor():
        this_node_input_index: List[int] = node_input_index[55]
        this_input_valid: bool = node_input_validity[55]
        if not this_input_valid:
            return
        ...
        for i in range(len(this_node_input_index)):
            ...
            if condition_0:
                ...
                x7 = x2*x5 + x3*x4
                # End of temp variables
                this_solution[10] = math.atan2(r_11*x6 - r_21*x7 + r_31*x1, r_12*x6 - r_22*x7 + r_32*x1)
                solution_queue[node_input_i_idx_in_queue] = this_solution
    ...
    def ExplicitSolutionNode_node_57_solve_th_4_processor():
    ...
    def ExplicitSolutionNode_node_59_solve_th_6_processor():
        this_node_input_index: List[int] = node_input_index[58]
        this_input_valid: bool = node_input_validity[58]
        if not this_input_valid:
            return
        ...
        for i in range(len(this_node_input_index)):
            ...
            th_4th_6_soa = this_solution[10]
            condition_0: bool = (1 >= zero_tolerance)
            if condition_0:
                # Temp variable for efficiency
                # End of temp variables
                this_solution[12] = -th_4 + th_4th_6_soa
                solution_queue[node_input_i_idx_in_queue] = this_solution
            else:
                queue_element_validity[node_input_i_idx_in_queue] = False
    ...
    def ExplicitSolutionNode_node_50_solve_negative_th_6_positive_th_4__soa_processor():
    ...
    def ExplicitSolutionNode_node_52_solve_th_4_processor():
    ...
    def ExplicitSolutionNode_node_54_solve_th_6_processor():
        ...
        for i in range(len(this_node_input_index)):
            ...
            if condition_0:
                # Temp variable for efficiency
                # End of temp variables
                this_solution[12] = -negative_th_6_positive_th_4__soa + th_4
                solution_queue[node_input_i_idx_in_queue] = this_solution
            else:
                queue_element_validity[node_input_i_idx_in_queue] = False
    ...
    def ExplicitSolutionNode_node_18_solve_th_4_processor():
        ...
        for i in range(len(this_node_input_index)):
            ...
            if condition_0:
                ...
                x6 = math.cos(th_1)
                x7 = x2*x6
                x8 = 4.50777232351566*x1
                # End of temp variables
                this_solution[9] = math.atan2(x0*(r_13*(x3*x4 - x5*x7) - r_23*(x3*x7 + x4*x5) + r_33*x1*x2), x0*(r_13*x5*x8 + r_23*x3*x8 + 4.50777232351566*r_33*x6 + 4.39545348293595*math.cos(th_5)))
                solution_queue[node_input_i_idx_in_queue] = this_solution
    ...
    def ExplicitSolutionNode_node_20_solve_th_2th_4th_5_soa_processor():
        ...
        for i in range(len(this_node_input_index)):
            ...
            if condition_0:
                # Temp variable for efficiency
                # End of temp variables
                this_solution[6] = th_2 + th_4 + th_5
                solution_queue[node_input_i_idx_in_queue] = this_solution
    ...
    def ExplicitSolutionNode_node_22_solve_th_0th_2th_4_soa_processor():
    ...
    def ExplicitSolutionNode_node_24_solve_th_1th_2th_4_soa_processor():
    ...
    def ExplicitSolutionNode_node_26_solve_th_6_processor():
        ...
        for i in range(len(this_node_input_index)):
            ...
            th_4 = this_solution[9]
            th_5 = this_solution[11]
            condition_0: bool = (1 >= zero_tolerance) or (abs(-r_11*((((-0.221839065558681*math.sin(th_5) + 0.975083293361163*math.cos(th_4)*math.cos(th_5))*math.cos(th_2) + math.sin(th_2)*math.sin(th_4)*math.cos(th_5))*math.cos(th_1) - (0.975083293361163*math.sin(th_5) + 0.221839065558681*math.cos(th_4)*math.cos(th_5))*math.sin(th_1))*math.cos(th_0) - ((0.221839065558681*math.sin(th_5) - 0.975083293361163*math.cos(th_4)*math.cos(th_5))*math.sin(th_2) + math.sin(th_4)*math.cos(th_2)*math.cos(th_5))*math.sin(th_0)) - r_21*((((-0.221839065558681*math.sin(th_5) + 0.975083293361163*math.cos(th_4)*math.cos(th_5))*math.cos(th_2) + math.sin(th_2)*math.sin(th_4)*math.cos(th_5))*math.cos(th_1) - (0.975083293361163*math.sin(th_5) + 0.221839065558681*math.cos(th_4)*math.cos(th_5))*math.sin(th_1))*math.sin(th_0) + ((0.221839065558681*math.sin(th_5) - 0.975083293361163*math.cos(th_4)*math.cos(th_5))*math.sin(th_2) + math.sin(th_4)*math.cos(th_2)*math.cos(th_5))*math.cos(th_0)) - r_31*((-(-0.221839065558681*math.sin(th_5) + 0.975083293361163*math.cos(th_4)*math.cos(th_5))*math.cos(th_2) - math.sin(th_2)*math.sin(th_4)*math.cos(th_5))*math.sin(th_1) - (0.975083293361163*math.sin(th_5) + 0.221839065558681*math.cos(th_4)*math.cos(th_5))*math.cos(th_1))) >= zero_tolerance) or (abs(-r_12*((((-0.221839065558681*math.sin(th_5) + 0.975083293361163*math.cos(th_4)*math.cos(th_5))*math.cos(th_2) + math.sin(th_2)*math.sin(th_4)*math.cos(th_5))*math.cos(th_1) - (0.975083293361163*math.sin(th_5) + 0.221839065558681*math.cos(th_4)*math.cos(th_5))*math.sin(th_1))*math.cos(th_0) - ((0.221839065558681*math.sin(th_5) - 0.975083293361163*math.cos(th_4)*math.cos(th_5))*math.sin(th_2) + math.sin(th_4)*math.cos(th_2)*math.cos(th_5))*math.sin(th_0)) - r_22*((((-0.221839065558681*math.sin(th_5) + 0.975083293361163*math.cos(th_4)*math.cos(th_5))*math.cos(th_2) + math.sin(th_2)*math.sin(th_4)*math.cos(th_5))*math.cos(th_1) - (0.975083293361163*math.sin(th_5) + 0.221839065558681*math.cos(th_4)*math.cos(th_5))*math.sin(th_1))*math.sin(th_0) + ((0.221839065558681*math.sin(th_5) - 0.975083293361163*math.cos(th_4)*math.cos(th_5))*math.sin(th_2) + math.sin(th_4)*math.cos(th_2)*math.cos(th_5))*math.cos(th_0)) - r_32*((-(-0.221839065558681*math.sin(th_5) + 0.975083293361163*math.cos(th_4)*math.cos(th_5))*math.cos(th_2) - math.sin(th_2)*math.sin(th_4)*math.cos(th_5))*math.sin(th_1) - (0.975083293361163*math.sin(th_5) + 0.221839065558681*math.cos(th_4)*math.cos(th_5))*math.cos(th_1))) >= zero_tolerance)
            if condition_0:
                # Temp variable for efficiency
                x0 = math.cos(th_1)
                x1 = math.sin(th_5)
                x2 = math.cos(th_5)
                x3 = x2*math.cos(th_4)
                x4 = 0.975083293361163*x1 + 0.221839065558681*x3
                x5 = math.sin(th_1)
                x6 = math.sin(th_2)
                x7 = x2*math.sin(th_4)
                x8 = x6*x7
                x9 = math.cos(th_2)
                x10 = 0.221839065558681*x1
                x11 = 0.975083293361163*x3
                x12 = x9*(-x10 + x11)
                x13 = -x0*x4 + x5*(-x12 - x8)
                x14 = math.cos(th_0)
                x15 = x6*(x10 - x11) + x7*x9
                x16 = math.sin(th_0)
                x17 = x0*(x12 + x8) - x4*x5
                x18 = x14*x15 + x16*x17
                x19 = x14*x17 - x15*x16
                # End of temp variables
                this_solution[12] = math.atan2(-r_12*x19 - r_22*x18 - r_32*x13, r_11*x19 + r_21*x18 + r_31*x13)
                solution_queue[node_input_i_idx_in_queue] = this_solution
    ...
    def ExplicitSolutionNode_node_28_solve_th_2th_4th_6_soa_processor():
        ...
        for i in range(len(this_node_input_index)):
            ...
            th_2 = this_solution[4]
            th_4 = this_solution[9]
            th_6 = this_solution[12]
            condition_0: bool = (1 >= zero_tolerance)
            if condition_0:
                # Temp variable for efficiency
                # End of temp variables
                this_solution[7] = th_2 + th_4 + th_6
                solution_queue[node_input_i_idx_in_queue] = this_solution
            else:
                queue_element_validity[node_input_i_idx_in_queue] = False
    ...
    def ExplicitSolutionNode_node_3_solve_th_2_processor():
        ...
        for i in range(len(this_node_input_index)):
            ...
            this_solution = solution_queue[node_input_i_idx_in_queue]
            th_3 = this_solution[8]
            condition_0: bool = (abs((Px*math.sin(th_0) - Py*math.cos(th_0))/(a_3*math.cos(th_3) + a_3 - d_2*math.sin(th_3))) <= 1)
            if condition_0:
                # Temp variable for efficiency
                x0 = safe_asin((-Px*math.sin(th_0) + Py*math.cos(th_0))/(-a_3*math.cos(th_3) - a_3 + d_2*math.sin(th_3)))
                # End of temp variables
                solution_0: IkSolution = copy.copy(this_solution)
                solution_0[4] = x0
                appended_idx = append_solution_to_queue(solution_0)
            ...
            condition_1: bool = (abs((Px*math.sin(th_0) - Py*math.cos(th_0))/(a_3*math.cos(th_3) + a_3 - d_2*math.sin(th_3))) <= 1)
            if condition_1:
                # Temp variable for efficiency
                x0 = safe_asin((-Px*math.sin(th_0) + Py*math.cos(th_0))/(-a_3*math.cos(th_3) - a_3 + d_2*math.sin(th_3)))
                # End of temp variables
    ...
    def ExplicitSolutionNode_node_5_solve_th_1_processor():
        ...
        for i in range(len(this_node_input_index)):
            ...
            if condition_0:
                ...
                x0 = math.sin(th_3)
                x1 = math.cos(th_3)
                x2 = (-a_3*x1 - a_3 + d_2*x0)*math.cos(th_2)
                x3 = -Px*math.cos(th_0) - Py*math.sin(th_0)
                x4 = a_3*x0 + d_2*x1 - d_2
                # End of temp variables
                this_solution[2] = math.atan2(Pz*x2 - x3*x4, Pz*x4 + x2*x3)
                solution_queue[node_input_i_idx_in_queue] = this_solution
    ...
    def ExplicitSolutionNode_node_45_solve_th_4th_6_soa_processor():
    ...
    def ExplicitSolutionNode_node_47_solve_th_4_processor():
    ...
    def ExplicitSolutionNode_node_49_solve_th_6_processor():
    ...
    def ExplicitSolutionNode_node_40_solve_negative_th_6_positive_th_4__soa_processor():
    ...
    def ExplicitSolutionNode_node_42_solve_th_4_processor():
        this_node_input_index: List[int] = node_input_index[41]
        this_input_valid: bool = node_input_validity[41]
        if not this_input_valid:
            return
    ...
    def ExplicitSolutionNode_node_44_solve_th_6_processor():
    ...
    def ExplicitSolutionNode_node_9_solve_th_4_processor():
        ...
        for i in range(len(this_node_input_index)):
            ...
            if condition_0:
                ...
                x12 = x1*x8 + x10*x6
                # End of temp variables
                this_solution[9] = math.atan2(x0*(r_13*(x3*x4 - x5*x7) - r_23*(x3*x7 + x4*x5) + r_33*x1*x2), x0*(-r_13*(x11*x3 + x12*x5) - r_23*(-x11*x5 + x12*x3) - r_33*(-x1*x10 + x6*x8)))
                solution_queue[node_input_i_idx_in_queue] = this_solution
    ...
    def ExplicitSolutionNode_node_11_solve_th_6_processor():
        ...
        for i in range(len(this_node_input_index)):
            ...
            if condition_0:
                ...
                x15 = -x11 + x12
                # End of temp variables
                this_solution[12] = math.atan2(x0*(r_12*(x13*x7 - x14) + r_22*(x10*x13 + x9) + r_32*x6), x0*(r_11*(x14 + x15*x7) + r_21*(x10*x15 - x9) - r_31*x6))
                solution_queue[node_input_i_idx_in_queue] = this_solution
            else:
                queue_element_validity[node_input_i_idx_in_queue] = False
    ...
    for i in range(len(solution_queue)):
        ...
        value_at_5 = ik_out_i[11]  # th_5
        new_ik_i[5] = value_at_5
        value_at_6 = ik_out_i[12]  # th_6
        new_ik_i[6] = value_at_6
        ik_out.append(new_ik_i)
    return ik_out


def rainbow_y1_r_arm_ik_solve(T_ee: np.ndarray, th_0):
    T_ee_raw_in = rainbow_y1_r_arm_ik_target_original_to_raw(T_ee)
    ik_output_raw = rainbow_y1_r_arm_ik_solve_raw(T_ee_raw_in, th_0 + th_0_offset_original2raw)
    ik_output = list()
...
def test_ik_solve_rainbow_y1_r_arm():
    ...
    ik_output = rainbow_y1_r_arm_ik_solve(ee_pose, th_0=theta_in[0])
