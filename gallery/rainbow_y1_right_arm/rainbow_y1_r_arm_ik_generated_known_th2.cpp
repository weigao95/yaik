...
static void computeRawIK(const Eigen::Matrix4d& T_ee, double th_2, SolutionQueue<intermediate_solution_size, max_n_solutions>& solution_queue, NodeIndexWorkspace<max_n_solutions>& node_index_workspace, std::vector<std::array<double, robot_nq>>& ik_output)
{
    ...
    auto EquationAllZeroDispatcherNode_node_2_processor = [&]() -> void
    {
        ...
        for(int i = 0; i < this_node_input_index.size(); i++)
        {
            ...
            const bool checked_result = std::fabs(Pz) <= 9.9999999999999995e-7 && std::fabs(a_3*std::sin(th_3) + d_2*std::cos(th_3) - d_2) <= 9.9999999999999995e-7 && std::fabs(a_3*std::cos(th_2)*std::cos(th_3) + a_3*std::cos(th_2) - d_2*std::sin(th_3)*std::cos(th_2)) <= 9.9999999999999995e-7;
    ...
    auto ExplicitSolutionNode_node_3_solve_th_1_processor = [&]() -> void
    {
        ...
        for(int i = 0; i < this_node_input_index.size(); i++)
        {
            ...
            const bool condition_0 = std::fabs(Pz) >= zero_tolerance || std::fabs(a_3*std::sin(th_3) + d_2*std::cos(th_3) - d_2) >= zero_tolerance || std::fabs(a_3*std::cos(th_2)*std::cos(th_3) + a_3*std::cos(th_2) - d_2*std::sin(th_3)*std::cos(th_2)) >= zero_tolerance;
            if (condition_0)
            {
                // Temp variable for efficiency
                ...
                const double x4 = d_2*x0*x2 - x1*x3 - x1;
                const double x5 = a_3*x2 + d_2*x3 - d_2;
                const double x6 = std::atan2(x4, x5);
                const double x7 = safe_sqrt(-std::pow(Pz, 2) + std::pow(x4, 2) + std::pow(x5, 2));
                // End of temp variables
                RawSolution solution_0(this_solution);
                solution_0[2] = x6 + std::atan2(x7, Pz);
                int appended_idx = append_solution_to_queue(solution_0);
                add_input_index_to(4, appended_idx);
            }
            
            const bool condition_1 = std::fabs(Pz) >= zero_tolerance || std::fabs(a_3*std::sin(th_3) + d_2*std::cos(th_3) - d_2) >= zero_tolerance || std::fabs(a_3*std::cos(th_2)*std::cos(th_3) + a_3*std::cos(th_2) - d_2*std::sin(th_3)*std::cos(th_2)) >= zero_tolerance;
            if (condition_1)
            {
                // Temp variable for efficiency
                const double x0 = std::cos(th_2);
                ...
                const double x7 = safe_sqrt(-std::pow(Pz, 2) + std::pow(x4, 2) + std::pow(x5, 2));
                // End of temp variables
                const double tmp_sol_value = x6 + std::atan2(-x7, Pz);
    ...
    auto EquationAllZeroDispatcherNode_node_4_processor = [&]() -> void
    {
        ...
        for(int i = 0; i < this_node_input_index.size(); i++)
        {
            ...
            if (!solution_queue.is_solution_valid(node_input_i_idx_in_queue))
                continue;
            const auto& this_solution = solution_queue.get_solution(node_input_i_idx_in_queue);
            const bool checked_result = std::fabs(Px) <= 9.9999999999999995e-7 && std::fabs(Py) <= 9.9999999999999995e-7;
            if (!checked_result)  // To non-degenerate node
    ...
    auto ExplicitSolutionNode_node_5_solve_th_0_processor = [&]() -> void
    {
        ...
        for(int i = 0; i < this_node_input_index.size(); i++)
        {
            ...
            const bool condition_0 = std::fabs(Px) >= 9.9999999999999995e-7 || std::fabs(Py) >= 9.9999999999999995e-7;
            if (condition_0)
            {
                // Temp variable for efficiency
                ...
                const double x1 = a_3*x0;
                const double x2 = std::sin(th_3);
                const double x3 = d_2*x2;
                const double x4 = (a_3 + x1 - x3)*std::sin(th_2);
                const double x5 = std::sin(th_1);
                const double x6 = d_2*x5;
                const double x7 = std::cos(th_1)*std::cos(th_2);
                const double x8 = -a_3*x2*x5 - a_3*x7 - x0*x6 - x1*x7 + x3*x7 + x6;
                // End of temp variables
                const double tmp_sol_value = std::atan2(Px*x4 - Py*x8, -Px*x8 - Py*x4);
    ...
    for(int i = 0; i < solution_queue.size(); i++)
    {
        ...
        const double value_at_2 = th_2;  // th_2
...
static void computeRawIK(const Eigen::Matrix4d& T_ee_raw, double th_2, RawIKWorksace& workspace)
{
    workspace.raw_ik_out.clear();
    workspace.raw_ik_out.reserve(max_n_solutions);
    computeRawIK(T_ee_raw, th_2, workspace.solution_queue, workspace.node_index_workspace, workspace.raw_ik_out);
}

static void computeIKUnChecked(const Eigen::Matrix4d& T_ee, double th_2, RawIKWorksace& workspace, std::vector<std::array<double, robot_nq>>& ik_output)
{
    const Eigen::Matrix4d& T_ee_raw = endEffectorTargetOriginalToRaw(T_ee);
    computeRawIK(T_ee_raw, th_2 + th_2_offset_original2raw, workspace);
...
static void computeIK(const Eigen::Matrix4d& T_ee, double th_2, RawIKWorksace& workspace, std::vector<std::array<double, robot_nq>>& ik_output)
{
    const Eigen::Matrix4d& T_ee_raw = endEffectorTargetOriginalToRaw(T_ee);
    computeRawIK(T_ee_raw, th_2 + th_2_offset_original2raw, workspace);
    const auto& raw_ik_out = workspace.raw_ik_out;
    ...
    computeRawIK(T_ee_raw_disturbed, th_2 + th_2_offset_original2raw, workspace);
...
static std::vector<std::array<double, robot_nq>> computeIK(const Eigen::Matrix4d& T_ee, double th_2)
{
    std::vector<std::array<double, robot_nq>> ik_output;
    RawIKWorksace raw_ik_workspace;
    computeIK(T_ee, th_2, raw_ik_workspace, ik_output);
    return ik_output;
}
...
void test_ik_solve_rainbow_y1_r_arm()
{
    ...
    auto ik_output = rainbow_y1_r_arm_ik::computeIK(ee_pose, theta[2]);
    for(int i = 0; i < ik_output.size(); i++)
    {
