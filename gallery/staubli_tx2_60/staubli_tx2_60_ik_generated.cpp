...
// Robot parameters
static constexpr double a_0 = 0.29;
static constexpr double d_1 = 0.02;
static constexpr double d_2 = 0.31;
static constexpr double d_3 = 0.07;
static constexpr double pre_transform_special_symbol_23 = 0.375;
...
static Eigen::Matrix4d computeFK(const std::array<double, robot_nq>& theta_input_original)
{
    ...
    ee_pose_raw(0, 2) = x20;
    ee_pose_raw(0, 3) = -d_1*x1 + d_2*x16 + d_3*x20 + x21*x4;
    ee_pose_raw(1, 0) = -x0*x23 + x12*x26;
    ee_pose_raw(1, 1) = -x0*x26 - x12*x23;
    ee_pose_raw(1, 2) = x27;
    ee_pose_raw(1, 3) = d_1*x4 + d_2*x24 + d_3*x27 + x1*x21;
    ee_pose_raw(2, 0) = x0*x28*x3 + x12*x30;
    ee_pose_raw(2, 1) = -x0*x30 + x12*x28*x3;
    ee_pose_raw(2, 2) = x31;
    ee_pose_raw(2, 3) = -a_0*x5 + d_2*x29 + d_3*x31;
...
static void computeTwistJacobian(const std::array<double, robot_nq>& theta_input_original, Eigen::Matrix<double, 6, 6>& jacobian)
{
    ...
    const double x31 = 1.0*a_0*x4;
    const double x32 = pre_transform_special_symbol_23 - x31;
    const double x33 = d_2*x27 + pre_transform_special_symbol_23 - x31;
    const double x34 = a_0*x20 + d_1*x6;
    const double x35 = d_2*x21 + x34;
    const double x36 = d_3*x30 + x33;
    const double x37 = d_3*x24 + x35;
    const double x38 = a_0*x10 - d_1*x1;
    const double x39 = d_2*x11 + x38;
    const double x40 = d_3*x18 + x39;
    // End of temp variables
...
static void computeTransformPointJacobian(const std::array<double, robot_nq>& theta_input_original, Eigen::Vector3d& point_on_ee, Eigen::Matrix<double, 6, 6>& jacobian)
{
    ...
    const double x18 = d_2*x13 + pre_transform_special_symbol_23 - x6;
    const double x19 = a_0*x12*x14 + d_1*x2;
    ...
    const double x25 = 1.0*x10*x11*x14 - x14*x9;
    const double x26 = -x2*x24 - x21*x25;
    const double x27 = std::cos(th_4);
    const double x28 = std::sin(th_4);
    const double x29 = -x13*x27 - x22*x24*x28;
    const double x30 = -x17*x27 - x28*(-x2*x21 + x24*x25);
    const double x31 = d_3*x29 + x18;
    const double x32 = d_3*x30 + x20;
    const double x33 = 1.0*p_on_ee_x;
    const double x34 = 1.0*x14;
    const double x35 = p_on_ee_z*x34;
    const double x36 = x2*x4;
    const double x37 = x11*x2;
    const double x38 = -x10*x36 - x37*x8;
    const double x39 = a_0*x37 - d_1*x34;
    const double x40 = d_2*x38 + x39;
    const double x41 = 1.0*x1*x10*x11 - x36*x8;
    const double x42 = 1.0*x14*x24 - x21*x41;
    const double x43 = -x27*x38 - x28*(x21*x34 + x24*x41);
    const double x44 = d_3*x43 + x40;
    const double x45 = x0*x14 + x1*x33;
    // End of temp variables
    ...
    jacobian(0, 4) = p_on_ee_y*x23 + p_on_ee_z*x26 - x18*x26 - x20*x23;
    jacobian(0, 5) = -p_on_ee_y*x29 + p_on_ee_z*x30 + x29*x32 - x30*x31;
    jacobian(1, 0) = x33;
    jacobian(1, 1) = -pre_transform_special_symbol_23*x34 + x35;
    jacobian(1, 2) = -x34*x7 + x35;
    jacobian(1, 3) = p_on_ee_x*x13 - p_on_ee_z*x38 - x13*x40 + x18*x38;
    jacobian(1, 4) = -p_on_ee_x*x23 - p_on_ee_z*x42 + x18*x42 + x21*x22*x40;
    jacobian(1, 5) = p_on_ee_x*x29 - p_on_ee_z*x43 - x29*x44 + x31*x43;
    jacobian(2, 1) = -x45;
    jacobian(2, 2) = 1.0*x1*x39 + 1.0*x14*x19 - x45;
    jacobian(2, 3) = -p_on_ee_x*x17 + p_on_ee_y*x38 + x17*x40 - x20*x38;
    jacobian(2, 4) = -p_on_ee_x*x26 + p_on_ee_y*x42 - x20*x42 + x26*x40;
    jacobian(2, 5) = -p_on_ee_x*x30 + p_on_ee_y*x43 + x30*x44 - x32*x43;
    return;
}
...
static void computeRawIK(const Eigen::Matrix4d& T_ee, SolutionQueue<intermediate_solution_size, max_n_solutions>& solution_queue, NodeIndexWorkspace<max_n_solutions>& node_index_workspace, std::vector<std::array<double, robot_nq>>& ik_output)
{
    ...
    auto ExplicitSolutionNode_node_1_solve_th_0_processor = [&]() -> void
    {
        ...
        const bool condition_0 = std::fabs(d_1) >= zero_tolerance || std::fabs(Px - d_3*r_13) >= zero_tolerance || std::fabs(Py - d_3*r_23) >= zero_tolerance;
        if (condition_0)
        {
            // Temp variable for efficiency
            ...
            const double x3 = std::sqrt(-std::pow(d_1, 2) + std::pow(x0, 2) + std::pow(x1, 2));
            const double x4 = -d_1;
            // End of temp variables
            
            auto solution_0 = make_raw_solution();
            solution_0[1] = x2 + std::atan2(x3, x4);
            int appended_idx = append_solution_to_queue(solution_0);
            add_input_index_to(2, appended_idx);
        }
        
        const bool condition_1 = std::fabs(d_1) >= zero_tolerance || std::fabs(Px - d_3*r_13) >= zero_tolerance || std::fabs(Py - d_3*r_23) >= zero_tolerance;
        if (condition_1)
        {
            // Temp variable for efficiency
            const double x0 = Px - d_3*r_13;
            ...
            solution_1[1] = x2 + std::atan2(-x3, x4);
    ...
    auto ExplicitSolutionNode_node_3_solve_th_2_processor = [&]() -> void
    {
        ...
        for(int i = 0; i < this_node_input_index.size(); i++)
        {
            ...
            const bool condition_0 = (1.0/2.0)*std::fabs((std::pow(a_0, 2) + std::pow(d_1, 2) + std::pow(d_2, 2) - std::pow(Px - d_3*r_13, 2) - std::pow(Py - d_3*r_23, 2) - std::pow(Pz - d_3*r_33, 2))/(a_0*d_2)) <= 1;
            if (condition_0)
            {
                // Temp variable for efficiency
                const double x0 = std::asin((1.0/2.0)*(-std::pow(a_0, 2) - std::pow(d_1, 2) - std::pow(d_2, 2) + std::pow(Px - d_3*r_13, 2) + std::pow(Py - d_3*r_23, 2) + std::pow(Pz - d_3*r_33, 2))/(a_0*d_2));
                // End of temp variables
            ...
            const bool condition_1 = (1.0/2.0)*std::fabs((std::pow(a_0, 2) + std::pow(d_1, 2) + std::pow(d_2, 2) - std::pow(Px - d_3*r_13, 2) - std::pow(Py - d_3*r_23, 2) - std::pow(Pz - d_3*r_33, 2))/(a_0*d_2)) <= 1;
            if (condition_1)
            {
                // Temp variable for efficiency
                const double x0 = std::asin((1.0/2.0)*(-std::pow(a_0, 2) - std::pow(d_1, 2) - std::pow(d_2, 2) + std::pow(Px - d_3*r_13, 2) + std::pow(Py - d_3*r_23, 2) + std::pow(Pz - d_3*r_33, 2))/(a_0*d_2));
    ...
    auto ExplicitSolutionNode_node_7_solve_th_1th_2_soa_processor = [&]() -> void
    {
...
void test_ik_solve_staubli_tx2_60()
{
    std::array<double, staubli_tx2_60_ik::robot_nq> theta;
    std::random_device rd;
    std::uniform_real_distribution<double> distribution;
    for(auto i = 0; i < theta.size(); i++)
        theta[i] = distribution(rd);
    const Eigen::Matrix4d ee_pose = staubli_tx2_60_ik::computeFK(theta);
    auto ik_output = staubli_tx2_60_ik::computeIK(ee_pose);
    for(int i = 0; i < ik_output.size(); i++)
    {
        Eigen::Matrix4d ee_pose_i = staubli_tx2_60_ik::computeFK(ik_output[i]);
