...
// Robot parameters
static constexpr double a_0 = 0.41;
static constexpr double a_1 = 1.12;
static constexpr double a_2 = 0.25;
static constexpr double d_3 = -1.285;
static constexpr double d_4 = -0.3;
static constexpr double pre_transform_special_symbol_23 = 0.94;
...
static Eigen::Matrix4d computeFK(const std::array<double, robot_nq>& theta_input_original)
{
    ...
    ee_pose_raw(0, 2) = x21;
    ee_pose_raw(0, 3) = a_0*x4 + a_2*x11 + d_3*x17 + d_4*x21 + x22*x4;
    ee_pose_raw(1, 0) = -x0*x24 + x13*x27;
    ee_pose_raw(1, 1) = -x0*x27 - x13*x24;
    ee_pose_raw(1, 2) = x28;
    ee_pose_raw(1, 3) = a_0*x1 + a_2*x23 + d_3*x25 + d_4*x28 + x1*x22;
    ee_pose_raw(2, 0) = x0*x30 + x13*x33;
    ee_pose_raw(2, 1) = -x0*x33 + x13*x30;
    ee_pose_raw(2, 2) = x34;
    ee_pose_raw(2, 3) = -a_1*x5 + a_2*x29 + d_3*x31 + d_4*x34;
...
static void computeTwistJacobian(const std::array<double, robot_nq>& theta_input_original, Eigen::Matrix<double, 6, 6>& jacobian)
{
    ...
    const double x30 = -a_1*x24 + pre_transform_special_symbol_23;
    const double x31 = a_2*x27 + d_3*x26 + x30;
    const double x32 = a_1*x7;
    const double x33 = a_0*x1 + x1*x32;
    const double x34 = a_2*x21 + d_3*x20 + x33;
    const double x35 = d_4*x29 + x31;
    const double x36 = d_4*x23 + x34;
    const double x37 = a_0*x5 + x32*x5;
    const double x38 = a_2*x13 + d_3*x10 + x37;
    const double x39 = d_4*x17 + x38;
    const double x40 = 1.0*a_0;
    // End of temp variables
    ...
    jacobian(3, 2) = x30*x5;
    jacobian(3, 3) = -x20*x31 + x26*x34;
    jacobian(3, 4) = -x22*x31 - x28*x34;
    jacobian(3, 5) = -x23*x35 + x29*x36;
    jacobian(4, 1) = -pre_transform_special_symbol_23*x1;
    jacobian(4, 2) = x1*x30;
    jacobian(4, 3) = x10*x31 - x26*x38;
    jacobian(4, 4) = x14*x31 + x28*x38;
    jacobian(4, 5) = x17*x35 - x29*x39;
    jacobian(5, 1) = std::pow(x0, 2)*x40 + std::pow(x4, 2)*x40;
    jacobian(5, 2) = -x1*x33 - x37*x5;
    jacobian(5, 3) = -x10*x34 + x20*x38;
    jacobian(5, 4) = -x14*x34 + x22*x38;
    jacobian(5, 5) = -x17*x36 + x23*x39;
    return;
}
...
static void computeTransformPointJacobian(const std::array<double, robot_nq>& theta_input_original, Eigen::Vector3d& point_on_ee, Eigen::Matrix<double, 6, 6>& jacobian)
{
    ...
    const double x22 = a_0*x21 + a_1*x11*x14;
    ...
    const double x36 = x2*x9;
    const double x37 = x2*x7;
    const double x38 = -x10*x37 + x36*x4;
    const double x39 = x10*x36 + x37*x4;
    const double x40 = a_0*x2 + a_1*x10*x2;
    const double x41 = a_2*x39 + d_3*x38 + x40;
    const double x42 = -x21*x26 - x24*x39;
    ...
    auto ExplicitSolutionNode_node_5_solve_th_1_processor = [&]() -> void
    {
        ...
        for(int i = 0; i < this_node_input_index.size(); i++)
        {
            ...
            if (condition_0)
            {
                ...
                const double x2 = std::cos(th_2);
                const double x3 = -a_1 - a_2*x2 + d_3*x1;
                const double x4 = a_2*x1 + d_3*x2;
                const double x5 = std::cos(th_0);
                const double x6 = std::sin(th_0);
                const double x7 = -Px*x5 - Py*x6 + a_0 + d_4*r_13*x5 + d_4*r_23*x6;
                // End of temp variables
...
void test_ik_solve_faunc_M900IB_700()
{
    std::array<double, faunc_M900IB_700_ik::robot_nq> theta;
    std::random_device rd;
    std::uniform_real_distribution<double> distribution;
    for(auto i = 0; i < theta.size(); i++)
        theta[i] = distribution(rd);
    const Eigen::Matrix4d ee_pose = faunc_M900IB_700_ik::computeFK(theta);
    auto ik_output = faunc_M900IB_700_ik::computeIK(ee_pose);
    for(int i = 0; i < ik_output.size(); i++)
    {
        Eigen::Matrix4d ee_pose_i = faunc_M900IB_700_ik::computeFK(ik_output[i]);
